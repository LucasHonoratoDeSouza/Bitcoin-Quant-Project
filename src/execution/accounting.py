...
class AccountingSystem:
    ...
    def generate_report(self):
        ...
        report = f"""
--- 📊 Paper Trading Report ({latest['date']}) ---
💰 Total Equity:   ${current:,.2f}
📈 ROI (Total):    {roi:+.2f}%
🏆 vs Buy & Hold:  {roi - bnh_roi:+.2f}% (Alpha)

--- Portfolio Composition ---
💵 Cash:           ${latest['cash']:,.2f} ({cash_pct:.1f}%)
🟠 BTC Value:      ${latest['btc_value']:,.2f} ({btc_pct:.1f}%) - {latest['btc_amount']:.6f} BTC
🔴 Debt:           ${latest['debt']:,.2f}
💸 Daily Interest: ${latest['interest_paid']:.2f}
---------------------------------------
"""
        return report
